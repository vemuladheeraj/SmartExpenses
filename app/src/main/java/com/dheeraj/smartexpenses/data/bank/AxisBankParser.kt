 ...
 * 
 * Supported formats:
 * - Debit: "Your Axis Bank Card ending 1234 has been charged Rs.500/-"
 * - Credit: "Your Axis Bank a/c XX1234 has been credited with Rs.10000.00"
 * - UPI, IMPS, and other transaction types
 * 
 * Common senders: AXIS, AXISB, variations with DLT patterns
 */
class AxisBankParser : BankParser() {
    ...
    override fun canHandle(sender: String): Boolean {
        ...
        val axisSenders = setOf(
            "AXIS",
            "AXISB",
            "AXISBANK"
        )
        
        if (upperSender in axisSenders) return true
        
        return CompiledPatterns.Axis.DLT_PATTERNS.any { it.matches(upperSender) }
    }
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        // Try Axis specific patterns
        if (message.contains("UPI", ignoreCase = true)) {
            CompiledPatterns.Axis.UPI_PATTERN.find(message)?.let { match ->
        ...
        if (message.contains(" at ", ignoreCase = true)) {
            CompiledPatterns.Axis.MERCHANT_PATTERN.find(message)?.let { match ->
                val merchant = match.groupValues[1].trim()
    ...
    override fun isTransactionMessage(message: String): Boolean {
        ...
        // Axis specific transaction keywords
        val axisTransactionKeywords = listOf(
            "charged", "debited", "credited", "withdrawn", "deposited",
            "spent", "received", "transferred", "paid", 
            "purchase", "deducted"
        )
        
        return axisTransactionKeywords.any { lowerMessage.contains(it) }
    }
}