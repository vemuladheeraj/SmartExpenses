 ...
 * 
 * Supported formats:
 * - Debit: "Your Federal Bank a/c XX1234 has been debited Rs.500/-"
 * - Credit: "Your Federal Bank a/c XX1234 has been credited Rs.10000.00"
 * - UPI, IMPS, and other transaction types
 * 
 * Common senders: FEDERAL, FEDERALB, variations with DLT patterns
 */
class FederalBankParser : BankParser() {
    ...
    override fun canHandle(sender: String): Boolean {
        ...
        val federalSenders = setOf(
            "FEDERAL",
            "FEDERALB",
            "FEDERALBANK"
        )
        
        if (upperSender in federalSenders) return true
        
        // Check for DLT patterns
        val dltPatterns = listOf(
            Regex("""FEDERAL[A-Z0-9]{6,}"""),
            Regex("""FEDERALB[A-Z0-9]{5,}""")
        )
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        // Try Federal Bank specific patterns
    ...
    override fun isTransactionMessage(message: String): Boolean {
        ...
        val federalTransactionKeywords = listOf(
            "debited", "credited", "withdrawn", "deposited",
            "spent", "received", "transferred", "paid", 
            "charged", "purchase", "deducted"
        )
        
        return federalTransactionKeywords.any { lowerMessage.contains(it) }
    }
}