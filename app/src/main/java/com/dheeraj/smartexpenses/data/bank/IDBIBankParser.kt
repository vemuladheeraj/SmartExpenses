 ...
 * 
 * Supported formats:
 * - Debit: "Your IDBI Bank a/c XX1234 has been debited Rs.500/-"
 * - Credit: "Your IDBI Bank a/c XX1234 has been credited Rs.10000.00"
 * - UPI, IMPS, and other transaction types
 * 
 * Common senders: IDBI, IDBIB, variations with DLT patterns
 */
class IDBIBankParser : BankParser() {
    ...
    override fun canHandle(sender: String): Boolean {
        ...
        val idbiSenders = setOf(
            "IDBI",
            "IDBIB",
            "IDBIBANK"
        )
        
        if (upperSender in idbiSenders) return true
        
        // Check for DLT patterns
        val dltPatterns = listOf(
            Regex("""IDBI[A-Z0-9]{6,}"""),
            Regex("""IDBIB[A-Z0-9]{5,}""")
        )
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        // Try IDBI Bank specific patterns
    ...
    override fun isTransactionMessage(message: String): Boolean {
        ...
        // IDBI Bank specific transaction keywords
        val idbiTransactionKeywords = listOf(
            "debited", "credited", "withdrawn", "deposited",
            "spent", "received", "transferred", "paid", 
            "charged", "purchase", "deducted"
        )
        
        return idbiTransactionKeywords.any { lowerMessage.contains(it) }
    }
}