...
/**
 * Parser for Juspay payment gateway SMS messages
 * 
 * Supported formats:
 * - UPI transactions through Juspay
 * - Payment confirmations
 * - Transaction notifications
 * 
 * Common senders: JUSPAY, variations with DLT patterns
 */
class JuspayParser : BankParser() {
    
    override fun getBankName() = "Juspay"
    
    override fun canHandle(sender: String): Boolean {
        val upperSender = sender.uppercase()
        
        val juspaySenders = setOf(
            "JUSPAY",
            "JUSPAYPG"
        )
        
        if (upperSender in juspaySenders) return true
        
        // Check for DLT patterns
        val dltPatterns = listOf(
            Regex("""JUSPAY[A-Z0-9]{6,}"""),
            Regex("""JUSPAYPG[A-Z0-9]{5,}""")
        )
    ...
    override fun extractChannel(message: String): String? {
        ...
        return when {
            ...
            else -> Constants.Channels.UPI // Default to UPI for Juspay
        }
    }
    ...
    override fun isTransactionMessage(message: String): Boolean {
        ...
        // Juspay specific transaction keywords
        val juspayTransactionKeywords = listOf(
            "debited", "credited", "withdrawn", "deposited",
            "spent", "received", "transferred", "paid", 
            "charged", "purchase", "deducted", "upi"
        )
        
        return juspayTransactionKeywords.any { lowerMessage.contains(it) }
    }
}