...
/**
 * Parser for Kotak Mahindra Bank SMS messages
 * 
 * Supported formats:
 * - Debit: "Your Kotak Bank a/c XX1234 has been debited Rs.500/-"
 * - Credit: "Your Kotak Bank a/c XX1234 has been credited Rs.10000.00"
 * - UPI, IMPS, and other transaction types
 * 
 * Common senders: KOTAK, KOTAKB, variations with DLT patterns
 */
class KotakBankParser : BankParser() {
    
    override fun getBankName() = "Kotak Mahindra Bank"
    
    override fun canHandle(sender: String): Boolean {
        val upperSender = sender.uppercase()
        
        val kotakSenders = setOf(
            "KOTAK",
            "KOTAKB",
            "KOTAKBANK"
        )
        
        if (upperSender in kotakSenders) return true
        
        // Check for DLT patterns
        val dltPatterns = listOf(
            Regex("""KOTAK[A-Z0-9]{6,}"""),
            Regex("""KOTAKB[A-Z0-9]{5,}""")
        )
