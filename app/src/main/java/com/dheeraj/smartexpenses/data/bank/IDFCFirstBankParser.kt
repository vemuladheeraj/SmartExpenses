 ...
 * 
 * Supported formats:
 * - Debit: "Your IDFC First Bank a/c XX1234 has been debited Rs.500/-"
 * - Credit: "Your IDFC First Bank a/c XX1234 has been credited Rs.10000.00"
 * - UPI, IMPS, and other transaction types
 * 
 * Common senders: IDFC, IDFCFIRST, variations with DLT patterns
 */
class IDFCFirstBankParser : BankParser() {
    ...
    override fun canHandle(sender: String): Boolean {
        ...
        val idfcSenders = setOf(
            "IDFC",
            "IDFCFIRST",
            "IDFCFIRSTBANK"
        )
        
        if (upperSender in idfcSenders) return true
        
        // Check for DLT patterns
        val dltPatterns = listOf(
            Regex("""IDFC[A-Z0-9]{6,}"""),
            Regex("""IDFCFIRST[A-Z0-9]{5,}""")
        )
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        // Try IDFC First Bank specific patterns
    ...
    override fun isTransactionMessage(message: String): Boolean {
        ...
        // IDFC First Bank specific transaction keywords
        val idfcTransactionKeywords = listOf(
            "debited", "credited", "withdrawn", "deposited",
            "spent", "received", "transferred", "paid", 
            "charged", "purchase", "deducted"
        )
        
        return idfcTransactionKeywords.any { lowerMessage.contains(it) }
    }
}
