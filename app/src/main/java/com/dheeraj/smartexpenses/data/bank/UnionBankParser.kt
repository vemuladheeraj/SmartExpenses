 ...
 * 
 * Supported formats:
 * - Debit: "Your Union Bank a/c XX1234 has been debited Rs.500/-"
 * - Credit: "Your Union Bank a/c XX1234 has been credited Rs.10000.00"
 * - UPI, IMPS, and other transaction types
 * 
 * Common senders: UNION, UNIONB, variations with DLT patterns
 */
class UnionBankParser : BankParser() {
    ...
    override fun canHandle(sender: String): Boolean {
        ...
        val unionSenders = setOf(
            "UNION",
            "UNIONB",
            "UNIONBANK"
        )
        
        if (upperSender in unionSenders) return true
        
        // Check for DLT patterns
        val dltPatterns = listOf(
            Regex("""UNION[A-Z0-9]{6,}"""),
            Regex("""UNIONB[A-Z0-9]{5,}""")
        )
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        // Try Union Bank specific patterns
    ...
    override fun isTransactionMessage(message: String): Boolean {
        ...
        // Union Bank specific transaction keywords
        val unionTransactionKeywords = listOf(
            "debited", "credited", "withdrawn", "deposited",
            "spent", "received", "transferred", "paid", 
            "charged", "purchase", "deducted"
        )
        
        return unionTransactionKeywords.any { lowerMessage.contains(it) }
    }
}