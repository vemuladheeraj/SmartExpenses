...
/**
 * Parser for Central Bank of India SMS messages
 * 
 * Supported formats:
 * - Debit: "Your Central Bank a/c XX1234 has been debited Rs.500/-"
 * - Credit: "Your Central Bank a/c XX1234 has been credited Rs.10000.00"
 * - UPI, IMPS, and other transaction types
 * 
 * Common senders: CENTRAL, CENTRALB, variations with DLT patterns
 */
class CentralBankOfIndiaParser : BankParser() {
    ...
    override fun canHandle(sender: String): Boolean {
        ...
        val centralSenders = setOf(
            "CENTRAL",
            "CENTRALB",
            "CENTRALBANK"
        )
        
        if (upperSender in centralSenders) return true
        
        // Check for DLT patterns
        val dltPatterns = listOf(
            Regex("""CENTRAL[A-Z0-9]{6,}"""),
            Regex("""CENTRALB[A-Z0-9]{5,}""")
        )
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        // Try Central Bank specific patterns
    ...
    override fun isTransactionMessage(message: String): Boolean {
        ...
        // Central Bank specific transaction keywords
        val centralTransactionKeywords = listOf(
            "debited", "credited", "withdrawn", "deposited",
            "spent", "received", "transferred", "paid", 
            "charged", "purchase", "deducted"
        )
        
        return centralTransactionKeywords.any { lowerMessage.contains(it) }
    }
}