 ...
 * 
 * Supported formats:
 * - Debit: "Your HSBC Bank a/c XX1234 has been debited Rs.500/-"
 * - Credit: "Your HSBC Bank a/c XX1234 has been credited Rs.10000.00"
 * - UPI, IMPS, and other transaction types
 * 
 * Common senders: HSBC, HSBCB, variations with DLT patterns
 */
class HSBCBankParser : BankParser() {
    ...
    override fun canHandle(sender: String): Boolean {
        ...
        val hsbcSenders = setOf(
            "HSBC",
            "HSBCB",
            "HSBCBANK"
        )
        
        if (upperSender in hsbcSenders) return true
        
        // Check for DLT patterns
        val dltPatterns = listOf(
            Regex("""HSBC[A-Z0-9]{6,}"""),
            Regex("""HSBCB[A-Z0-9]{5,}""")
        )
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        // Try HSBC Bank specific patterns
    ...
    override fun isTransactionMessage(message: String): Boolean {
        ...
        // HSBC Bank specific transaction keywords
        val hsbcTransactionKeywords = listOf(
            "debited", "credited", "withdrawn", "deposited",
            "spent", "received", "transferred", "paid", 
            "charged", "purchase", "deducted"
        )
        
        return hsbcTransactionKeywords.any { lowerMessage.contains(it) }
    }
}