 ...
 * 
 * Common senders: SBI, SBIN, variations with DLT patterns
 */
class SBIBankParser : BankParser() {
    ...
    override fun canHandle(sender: String): Boolean {
        ...
        val sbiSenders = setOf(
            "SBI",
            "SBIN",
            "SBIBANK"
        )
        
        if (upperSender in sbiSenders) return true
        
        return CompiledPatterns.SBI.DLT_PATTERNS.any { it.matches(upperSender) }
    }
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        // Try SBI specific patterns
        if (message.contains("UPI", ignoreCase = true)) {
            CompiledPatterns.SBI.UPI_PATTERN.find(message)?.let { match ->
        ...
        if (message.contains(" to ", ignoreCase = true)) {
            CompiledPatterns.SBI.MERCHANT_PATTERN.find(message)?.let { match ->
                val merchant = match.groupValues[1].trim()
    ...
    override fun isTransactionMessage(message: String): Boolean {
        ...
        // SBI specific transaction keywords
        val sbiTransactionKeywords = listOf(
            "debited", "credited", "withdrawn", "deposited",
            "spent", "received", "transferred", "paid", 
            "charged", "purchase"
        )
        
        return sbiTransactionKeywords.any { lowerMessage.contains(it) }
    }
}