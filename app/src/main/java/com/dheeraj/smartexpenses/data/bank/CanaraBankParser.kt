 ...
 * 
 * Supported formats:
 * - Debit: "Your Canara Bank a/c XX1234 has been debited Rs.500/-"
 * - Credit: "Your Canara Bank a/c XX1234 has been credited Rs.10000.00"
 * - UPI, IMPS, and other transaction types
 * 
 * Common senders: CANARA, CANARAB, variations with DLT patterns
 */
class CanaraBankParser : BankParser() {
    ...
    override fun canHandle(sender: String): Boolean {
        ...
        val canaraSenders = setOf(
            "CANARA",
            "CANARAB",
            "CANARABANK"
        )
        
        if (upperSender in canaraSenders) return true
        
        // Check for DLT patterns
        val dltPatterns = listOf(
            Regex("""CANARA[A-Z0-9]{6,}"""),
            Regex("""CANARAB[A-Z0-9]{5,}""")
        )
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        // Try Canara Bank specific patterns
    ...
    override fun isTransactionMessage(message: String): Boolean {
        ...
        // Canara Bank specific transaction keywords
        val canaraTransactionKeywords = listOf(
            "debited", "credited", "withdrawn", "deposited",
            "spent", "received", "transferred", "paid", 
            "charged", "purchase", "deducted"
        )
        
        return canaraTransactionKeywords.any { lowerMessage.contains(it) }
    }
}