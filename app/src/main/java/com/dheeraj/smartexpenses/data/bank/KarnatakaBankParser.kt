 ...
 * 
 * Supported formats:
 * - Debit: "Your Karnataka Bank a/c XX1234 has been debited Rs.500/-"
 * - Credit: "Your Karnataka Bank a/c XX1234 has been credited Rs.10000.00"
 * - UPI, IMPS, and other transaction types
 * 
 * Common senders: KARNATAKA, KARNATAKAB, variations with DLT patterns
 */
class KarnatakaBankParser : BankParser() {
    ...
    override fun canHandle(sender: String): Boolean {
        ...
        val karnatakaSenders = setOf(
            "KARNATAKA",
            "KARNATAKAB",
            "KARNATAKABANK"
        )
        
        if (upperSender in karnatakaSenders) return true
        
        // Check for DLT patterns
        val dltPatterns = listOf(
            Regex("""KARNATAKA[A-Z0-9]{6,}"""),
            Regex("""KARNATAKAB[A-Z0-9]{5,}""")
        )
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        // Try Karnataka Bank specific patterns
    ...
    override fun isTransactionMessage(message: String): Boolean {
        ...
        // Karnataka Bank specific transaction keywords
        val karnatakaTransactionKeywords = listOf(
            "debited", "credited", "withdrawn", "deposited",
            "spent", "received", "transferred", "paid", 
            "charged", "purchase", "deducted"
        )
        
        return karnatakaTransactionKeywords.any { lowerMessage.contains(it) }
    }
}
