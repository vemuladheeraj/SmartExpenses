...
/**
 * Parser for Utkarsh Small Finance Bank SMS messages
 * 
 * Supported formats:
 * - Debit: "Your Utkarsh Bank a/c XX1234 has been debited Rs.500/-"
 * - Credit: "Your Utkarsh Bank a/c XX1234 has been credited Rs.10000.00"
 * - UPI, IMPS, and other transaction types
 * 
 * Common senders: UTKARSH, UTKARSHB, variations with DLT patterns
 */
class UtkarshBankParser : BankParser() {
    
    override fun getBankName() = "Utkarsh Small Finance Bank"
    
    override fun canHandle(sender: String): Boolean {
        val upperSender = sender.uppercase()
        
        val utkarshSenders = setOf(
            "UTKARSH",
            "UTKARSHB",
            "UTKARSHSFB"
        )
        
        if (upperSender in utkarshSenders) return true
        
        // Check for DLT patterns
        val dltPatterns = listOf(
            Regex("""UTKARSH[A-Z0-9]{6,}"""),
            Regex("""UTKARSHB[A-Z0-9]{5,}""")
        )
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        // Try Utkarsh Bank specific patterns
    ...
    override fun isTransactionMessage(message: String): Boolean {
        ...
        // Utkarsh Bank specific transaction keywords
        val utkarshTransactionKeywords = listOf(
            "debited", "credited", "withdrawn", "deposited",
            "spent", "received", "transferred", "paid", 
            "charged", "purchase", "deducted"
        )
        
        return utkarshTransactionKeywords.any { lowerMessage.contains(it) }
    }
}